...
func (ra *RegistrationAuthorityImpl) checkRegistrationLimit(ip net.IP) error {
	...
	if limit.Enabled() {
		...
		if count >= int(limit.GetThreshold(ip.String())) {
...
func (ra *RegistrationAuthorityImpl) NewRegistration(init core.Registration) (reg core.Registration, err error) {
	...
	// These fields aren't updatable by the end user, so they aren't copied by
	// MergeUpdate. But we need to fill them in for new registrations.
	reg.InitialIP = init.InitialIP
	reg.CreatedAt = ra.clk.Now()
